...
export const getAllProducts = async (req, res, next) => {
  ...
  try {
    ...
    if (price && price.gte && price.lte) {
      query.price = { $gte: parseInt(price.gte), $lte: parseInt(price.lte) };

    }
    if (rating && rating.gte && rating.lte) {
      query.rating = { $gte: parseInt(rating.gte), $lte: parseInt(rating.lte) };
    ...
    const products = await getAllProductsRepo(query, skip, pageSize);
...
export const deleteReview = async (req, res, next) => {
  ...
  try {
    ...
    //   // console.log("Checking condition", rev.user.toString() === req.user._id.toString());
    //   console.log("Checking condition", revUserId === reqUserId, "rev.user:", revUserId, "req.user._id:", reqUserId);
      return rev._id.toString() === reviewId.toString() && rev.user.toString() === req.user._id.toString() ;
      // console.log("Checking condition", rev.user == req.user._id);
    });
    if (isReviewExistIndex < 0) {
      return next(new ErrorHandler(400, "either review doesn't exist or You'r trying to delete other's User review"));
    }
